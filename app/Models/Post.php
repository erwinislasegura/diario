...
final class Post
{
    ...
    public static function save(array $d, ?int $id=null): int {
        $db=Database::connection(); $sourceUrl=$d['source_url']??null;$sourceName=$d['source_name']??null;$sourceHash=$sourceUrl?hash('sha256',$sourceUrl):null;
        $values=[$d['category_id'],$d['user_id'],$d['title'],$d['slug'],$d['excerpt'],$d['body'],$d['image'],$d['status'],$d['featured'],$d['published_at'],$sourceName,$sourceUrl,$sourceHash];
        if($id){$values[]=$id;$sql='UPDATE posts SET category_id=?,user_id=?,title=?,slug=?,excerpt=?,body=?,image=?,status=?,featured=?,published_at=?,source_name=?,source_url=?,source_hash=? WHERE id=?';}else{$sql='INSERT INTO posts(category_id,user_id,title,slug,excerpt,body,image,status,featured,published_at,source_name,source_url,source_hash) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?)';}
    ...
    public static function findBySourceUrl(string $url): ?array { $s=Database::connection()->prepare('SELECT id,title,slug,status FROM posts WHERE source_hash=? LIMIT 1');$s->execute([hash('sha256',$url)]);return $s->fetch()?:null; }
    public static function slugExists(string $slug): bool { $s=Database::connection()->prepare('SELECT 1 FROM posts WHERE slug=? LIMIT 1');$s->execute([$slug]);return (bool)$s->fetchColumn(); }
